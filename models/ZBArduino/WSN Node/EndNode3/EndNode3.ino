...
Il datalogging avviene mediante file EndNode3.txt
N.b.
...
#include <dht11.h>
const uint8_t dht11SensorsQty = 4; // Numero di sensori DHT11
const uint8_t dht11SensorsDIP [4] = {2,3,5,6}; // DIP cui sono connessi
dht11 dht11_1, dht11_2, dht11_3, dht11_4; // Istanzio i singoli oggetti
dht11 dht11Sensors[4] = {dht11_1, dht11_2, dht11_3, dht11_4}; // Dispongo gli oggetti in un array

///////////////////////////////////////////////////////////////////////////////
//  DS18B20 sensors

#include <OneWire.h>
const uint8_t ds18b20SensorsQty = 1; // Numero sonde ds18b20
const uint8_t ds18b20SensorsDIP [1] = {9}; // DIP cui sono connesse
OneWire ds18b20_1 (ds18b20SensorsDIP[0]); // Istanzio i singoli oggetti di tipo ds18b20 
OneWire ds18b20Sensors[] = {ds18b20_1}; // Dispongo gli oggetti in un array

///////////////////////////////////////////////////////////////////////////////
//  Co2 sensors

int Co2SensorAIP=A0;
const int DcGain=8.5; // Amplifier gain
...
void setup() {
  ...
  if(!SD.begin(chipSelect)) {
    Serial.println(F("*** E3 - M1")); // SD Initialization failed
    Serial.flush();
  }
//  initSession();
}
...
void storeData (String str){
  EndNode2LogFile = SD.open("EndNode3.txt", FILE_WRITE);
 
  if (EndNode2LogFile){ 
//    Serial.println(F("M2")); // Writing EndNode2.txt
//    Serial.flush();
    EndNode2LogFile.println(str); // Append new record to file
    EndNode2LogFile.close();
  }
  else {
    Serial.println(F("M3"));// ERROR: Writing EndNode2.txt failed
    Serial.flush(); 
  }  
}
...
void initSession() {
  ...
  String str="*** E3 - Starting new session: ";  
  str+=rtcModule.getTimeStamp();
  Serial.println(str);// Starting new session (M4) 
  Serial.flush();
  storeData(str);
}
*/
