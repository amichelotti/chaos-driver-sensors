...
Il datalogging avviene mediante file EndNode4.txt
N.b.
...
const int sampleInterval = 10000; // Intervallo di campionamento per ogni ciclo
...
void setup() {
  ...
  if(!SD.begin(chipSelect)) {
    Serial.println(F("*** E4 - M1")); // SD Initialization failed
    Serial.flush();
  }
//  initSession();
}
...
The End Node Device sends sensor data periodically
...
void storeData (String str){
  EndNode2LogFile = SD.open("EndNode4.txt", FILE_WRITE);
  ...
  if (EndNode2LogFile){ 
    ...
    Serial.println(F("*** E4 - M3"));// ERROR: Writing EndNode2.txt failed
    Serial.flush(); 
  }  
}
...
void initSession() {
  ...
  String str="*** E4 - Starting new session: ";
