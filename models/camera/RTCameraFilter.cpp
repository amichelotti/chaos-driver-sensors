...
RTCameraFilter::RTCameraFilter(
    const string                &_control_unit_id,
    const string                &_control_unit_param,
    const ControlUnitDriverList &_control_unit_drivers)
    : RTCameraBase(_control_unit_id, _control_unit_param, _control_unit_drivers, 0), remove_src(false), apply_gauss_fit(false) {
...
void RTCameraFilter::unitDefineActionAndDataset() {
  addAttributeToDataSet("REFX", "Reference X ", chaos::DataType::TYPE_DOUBLE, chaos::DataType::Input);
  addAttributeToDataSet("REFY", "Reference Y", chaos::DataType::TYPE_DOUBLE, chaos::DataType::Input);

  addAttributeToDataSet("REFSX", "Reference Sigma X ", chaos::DataType::TYPE_DOUBLE, chaos::DataType::Input);
  addAttributeToDataSet("REFSY", "Reference Sigma Y ", chaos::DataType::TYPE_DOUBLE, chaos::DataType::Input);
  addAttributeToDataSet("REFRHO", "Reference Tilt angle ", chaos::DataType::TYPE_DOUBLE, chaos::DataType::Input);

  addAttributeToDataSet("AMPLITUDE", "Estimated amplitude ", chaos::DataType::TYPE_DOUBLE, chaos::DataType::Output);
  addAttributeToDataSet("X", "Estimated X ", chaos::DataType::TYPE_DOUBLE, chaos::DataType::Output);
  addAttributeToDataSet("Y", "Estimated Y", chaos::DataType::TYPE_DOUBLE, chaos::DataType::Output);

  addAttributeToDataSet("SX", "Estimated Sigma X ", chaos::DataType::TYPE_DOUBLE, chaos::DataType::Output);
  addAttributeToDataSet("SY", "Estimated Sigma Y ", chaos::DataType::TYPE_DOUBLE, chaos::DataType::Output);
  addAttributeToDataSet("RHO", "Estimated Tilt angle ", chaos::DataType::TYPE_DOUBLE, chaos::DataType::Output);

  addStateVariable(StateVariableTypeAlarmCU, "out_of_reference", "estimated ellipse out of reference");

  addStateVariable(StateVariableTypeAlarmCU, "filtering_error", "Error performing filter");
...
int RTCameraFilter::filtering(cv::Mat &image) {
  try {
    ...
    if (!apply_gauss_fit) {
      ...
      if (apply_gauss_fit) {
        if ((S_x > 0) && (S_y > 0)) {
          ellipse(image, Point(X_m, Y_m), Size(S_x, S_y), rho, 0, 360, Scalar(128, 0, 0), 1, 8);
        }

        if ((*REFSX > 0) && (*REFSY > 0)) {
          setStateVariableSeverity(
              StateVariableTypeAlarmCU, "out_of_reference", chaos::common::alarm::MultiSeverityAlarmLevelClear);
          ellipse(image, Point(*REFSX, *REFSY), Size(*REFSX, *REFSX), *REFRHO, 0, 360, Scalar(255, 255, 255), 1, 8);

          if (checkEllipseInside(*REFX, *REFY, *REFRHO, *REFSX, *REFSY, X_m, Y_m, S_x, S_y, rho) == false) {
            setStateVariableSeverity(
                StateVariableTypeAlarmCU, "out_of_reference", chaos::common::alarm::MultiSeverityAlarmLevelHigh);
          }
        }
      }
