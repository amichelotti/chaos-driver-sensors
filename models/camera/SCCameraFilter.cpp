...
PUBLISHABLE_CONTROL_UNIT_IMPLEMENTATION(SCCameraFilter)

/*#define SCCameraFilterLAPP_		LAPP_ << "[SCCameraFilter] "
#define SCCameraFilterLDBG_		LDBG_ << "[SCCameraFilter] " <<
__PRETTY_FUNCTION__ << " " #define SCCameraFilterLERR_		LERR_ <<
"[SCCameraFilter] " << __PRETTY_FUNCTION__ << "("<<__LINE__<<") "
*/
#define SCCameraFilterLAPP_ CUAPP
#define SCCameraFilterLDBG_ CUDBG
#define SCCameraFilterLERR_ CUERR
/*
 Construct
 */
#define DECODE_CVENCODING(e, cvenc)                                            \
  if (e == #cvenc) {                                                           \
    SCCameraFilterLDBG_ << "Found Encoding:" << #cvenc;                          \
    framebuf_encoding = cvenc;                                                 \
  }

SCCameraFilter::SCCameraFilter(const string &_control_unit_id,
  ...
  SCCameraFilterLDBG_ << "Creating " << _control_unit_id
...
void SCCameraFilter::updateProperty() {
  ...
  for (std::vector<std::string>::iterator i = props.begin(); i != props.end();
    ...
    if (!camera_props.isCDataWrapperValue(*i)) {
      ...
      if (camera_props.getValueType(*i) == chaos::DataType::TYPE_DOUBLE) {
        ...
        SCCameraFilterLDBG_ << "Camera Property double " << *i
                          << " VALUE:" << tmp;
      ...
      if (camera_props.getValueType(*i) == chaos::DataType::TYPE_INT32) {
        int32_t tmp, *p;
        driver->getCameraProperty(*i, tmp);
        SCCameraFilterLDBG_ << "Camera Property int " << *i << " VALUE:" << tmp;
...
bool SCCameraFilter::setProp(const std::string &name, const std::string& value, uint32_t size){
  return false;
}

bool SCCameraFilter::setProp(const std::string &name, int32_t value,
                           uint32_t size) {
  int ret;
  int32_t valuer;
  SCCameraFilterLDBG_ << "SET IPROP:" << name << " VALUE:" << value;
  ...
  *p = valuer;
  SCCameraFilterLDBG_ << "SET IPROP:" << name << " SET VALUE:" << value
...
bool SCCameraFilter::setProp(const std::string &name, double value,
                           uint32_t size) {
  int ret;
  double valuer;
  SCCameraFilterLDBG_ << "SET FPROP:" << name << " VALUE:" << value << endl;
...
*/
void SCCameraFilter::unitDefineActionAndDataset() throw(chaos::CException) {
  ...
  addAttributeToDataSet(TRIGGER_HW_TIMEOUT_KEY, "Timeout applied when HW triggered",
                        chaos::DataType::TYPE_INT32, chaos::DataType::Output);
  addAttributeToDataSet(TRIGGER_SW_TIMEOUT_KEY, "Timeout applied when SW triggered",
                        chaos::DataType::TYPE_INT32, chaos::DataType::Output);
 ...
 addHandlerOnInputAttributeName<::driver::sensor::camera::SCCameraFilter,
                                       double>(
            this, &::driver::sensor::camera::SCCameraFilter::setProp, "ZOOMX");

addHandlerOnInputAttributeName<::driver::sensor::camera::SCCameraFilter,
                                       double>(
            this, &::driver::sensor::camera::SCCameraFilter::setProp, "ZOOMY");
  
addHandlerOnInputAttributeName<::driver::sensor::camera::SCCameraFilter,
                                       int32_t>(
            this, &::driver::sensor::camera::SCCameraFilter::setProp, "ROIX");

 addHandlerOnInputAttributeName<::driver::sensor::camera::SCCameraFilter,
                                       int32_t>(
            this, &::driver::sensor::camera::SCCameraFilter::setProp, "ROIY");

addHandlerOnInputAttributeName<::driver::sensor::camera::SCCameraFilter,
                                       int32_t>(
            this, &::driver::sensor::camera::SCCameraFilter::setProp, "ROIXSIZE");

addHandlerOnInputAttributeName<::driver::sensor::camera::SCCameraFilter,
                                       int32_t>(
            this, &::driver::sensor::camera::SCCameraFilter::setProp, "ROIYSIZE");
...
if(apply_moment){
  ...
  for (std::vector<std::string>::iterator i = props.begin(); i != props.end();
       i++) {
    if (!camera_props.isCDataWrapperValue(*i)) {
      SCCameraFilterLDBG_ << "ADDING ATTRIBUTE:" << *i
      ...
      if (camera_props.getValueType(*i) == chaos::DataType::TYPE_DOUBLE) {
        addHandlerOnInputAttributeName<::driver::sensor::camera::SCCameraFilter,
                                       double>(
            this, &::driver::sensor::camera::SCCameraFilter::setProp, *i);
      }
      if (camera_props.getValueType(*i) == chaos::DataType::TYPE_INT32) {
        addHandlerOnInputAttributeName<::driver::sensor::camera::SCCameraFilter,
                                       int32_t>(
...
void SCCameraFilter::unitDefineCustomAttribute() {
  std::string config;
  ...
  SCCameraFilterLDBG_ << "ADDING CONFIG:" << config;
...
void SCCameraFilter::unitInit() throw(chaos::CException) {
  int ret;
  int32_t itype;
  ...
  for (std::vector<std::string>::iterator i = props.begin(); i != props.end();
    ...
    if (!camera_props.isCDataWrapperValue(*i)) {
      ...
      if (camera_props.getValueType(*i) == chaos::DataType::TYPE_DOUBLE) {
        ...
        if(tmp!=0){
          /// to check!!!
          SCCameraFilterLDBG_ << "Init Double \"" << *i << "\" from input:" << tmp;
      ...
      if (camera_props.getValueType(*i) == chaos::DataType::TYPE_INT32) {
        ...
        if(tmp!=0){
          SCCameraFilterLDBG_ << "Init Integer \"" << *i << "\" from input:" << tmp;
  ...
  if (driver->getImageProperties(width, height, itype) == 0) {
    SCCameraFilterLDBG_ << "CAMERA IMAGE:" << width << "x" << height;
    *sizex = width;
    *sizey = height;
  }
  if ((*sizex == 0) || (*sizey == 0)) {
    SCCameraFilterLERR_ << "Invalid image sizes:" << *sizex << "x" << *sizey;
  ...
  SCCameraFilterLDBG_ << "Starting acquiring imagex " << *sizex << "x" << *sizey;
  if (*fmt == 0) {
    strcpy(encoding, ".png");
...
void SCCameraFilter::cameraGrabCallBack(const void *buf, uint32_t blen,
                                      uint32_t width, uint32_t heigth,
                                      uint32_t error) {}
void SCCameraFilter::startGrabbing() {
  SCCameraFilterLDBG_ << "Start Grabbing";
  metadataLogging(
  ...
  updateProperty();
  SCCameraFilterLDBG_ << "Allocated " << CAMERA_FRAME_BUFFERING << " buffers of "
                    << (*sizex * *sizey * 4) << " bytes";
  
  try{
    shared_mem.reset(new ::common::misc::data::SharedMem(getCUID(),(*sizex * *sizey * 4)));
    SCCameraFilterLDBG_ << "opened sharedMem \""<<shared_mem->getName()<<"\" at @"<<std::hex<<shared_mem->getMem()<<" size:"<<std::dec<<shared_mem->getSize();
  ...
  capture_th = boost::thread(&SCCameraFilter::captureThread, this);
  getAttributeCache()->setCustomDomainAsChanged();
  pushCustomDataset();
}

void SCCameraFilter::stopGrabbing() {
  stopCapture = true;
  metadataLogging(
      chaos::common::metadata_logging::StandardLoggingChannel::LogLevelInfo,
      "Stop grabbing");

  SCCameraFilterLDBG_ << "Stop Grabbing..." << stopCapture;
  ...
  setStateVariableSeverity(StateVariableTypeAlarmCU, "captureQueueFull",
                           chaos::common::alarm::MultiSeverityAlarmLevelClear);
  SCCameraFilterLDBG_ << "Stop Grabbing done" << stopCapture;
}
//! Execute the work, this is called with a determinated delay, it must be as
//! fast as possible
void SCCameraFilter::unitStart() throw(chaos::CException) { startGrabbing(); }

void SCCameraFilter::captureThread() {
  int ret;
  const char *img = 0;
  uint64_t start;
  counter_capture = 0;
  SCCameraFilterLDBG_ << "Capture thread STARTED";
  encode_th = boost::thread(&SCCameraFilter::encodeThread, this);
  ...
  */
          ...
          SCCameraFilterLDBG_ << "Capture FULL Queue:" << captureQueue
                            << " Waiting...";
       ...
       //   boost::chrono::system_clock::time_point wakeUpTime = boost::chrono::system_clock::now() + period;
          ...
          SCCameraFilterLDBG_ << "Capture Queue:" << captureQueue;
          captureQueue++;
          wait_capture.notify_one();
        }
      ...
      if (ret == TRIGGER_TIMEOUT_ERROR) {
        SCCameraFilterLERR_ << " wait returned:" << ret
        ...
        SCCameraFilterLERR_ << " Error wait returned:" << ret;
                                  setStateVariableSeverity(
            StateVariableTypeAlarmDEV, "capture_error",
  ...
  SCCameraFilterLDBG_ << "Capture thread exiting Queue: " << captureQueue;
  ...
  SCCameraFilterLDBG_ << "Capture thread ENDED Queue:" << captureQueue;
}
void SCCameraFilter::encodeThread() {
  uint64_t start;
  counter_encode = 0;
  SCCameraFilterLDBG_ << "Encode thread STARTED";
  while (!stopCapture) {
    ...
    if (captureImg.pop(a) && a.buf) {
          ...
          do {
            ...
            if (encoderet == false) {
             ...
             /* setStateVariableSeverity(
                  ...
                  chaos::common::alarm::MultiSeverityAlarmLevelWarning);*/
              SCCameraFilterLDBG_ << "Encode Queue FULL: " << encodeQueue
                                << " Waiting//";
      ...
      SCCameraFilterLDBG_ << "Encode EMPTY";
  ...
  SCCameraFilterLDBG_ << "Encode thread exiting Queue: " << encodeQueue;
  ...
  SCCameraFilterLDBG_ << "Encode thread ENDED Queue: " << encodeQueue;
}

//! Execute the Control Unit work
void SCCameraFilter::unitRun() throw(chaos::CException) {
  ...
  if ((counter_capture % 100000) == 0) {
    // SCCameraFilterLDBG_<<"CAPTURE TIME (us):"<<capture_time/counter_capture<<"
  ...
  if (encodedImg.pop(ele)) {
    a=ele.img;
    encodeQueue--;
  /*  SCCameraFilterLDBG_ << " Encode Queue:" << encodeQueue
    ...
    */
    ...
    // SCCameraFilterLDBG_<<"Encode EMPTY :"<<encodeWritePointer<<"
...
//! Execute the Control Unit work
void SCCameraFilter::unitStop() throw(chaos::CException) { stopGrabbing(); 
}

//! Deinit the Control Unit
void SCCameraFilter::unitDeinit() throw(chaos::CException) {

  driver->cameraDeinit();
}

//! pre imput attribute change
void SCCameraFilter::unitInputAttributePreChangeHandler() throw(
    chaos::CException) {}

//! attribute changed handler
void SCCameraFilter::unitInputAttributeChangedHandler() throw(chaos::CException) {
  ...
  //            SCCameraFilterLDBG_<<"writing output channel "<<*j<<", size
  //            :"<<input_size[*j];
  //        }
  //    }
...
/*
CDataWrapper *SCCameraFilter::my_custom_action(CDataWrapper *actionParam, bool&
