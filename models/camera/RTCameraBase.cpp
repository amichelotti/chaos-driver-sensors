    ...
    : RTAbstractControlUnit(_control_unit_id, _control_unit_param, _control_unit_drivers), encode_thds(ENCODE_THREADS), driver(NULL), streamer(NULL), capture_exited(true), capture_cnt(0), framebuf_encoding(CV_8UC3), buffering(_buffering), png_strategy(IMWRITE_PNG_STRATEGY_DEFAULT), encode_time(0), capture_time(0), network_time(0), hw_trigger_timeout_us(5000000), sw_trigger_timeout_us(0), imagesizex(0), imagesizey(0), apply_resize(false), trigger_timeout(5000), bpp(3), stopCapture(true), subImage(NULL), performCalib(false), applyCalib(false), cameraStreamEnable(true), calibrationImages(1), applyReference(false), refenceThick(2), refenceR(0), refenceG(255), refenceB(0)
{
  ...
  createProperty(
      "compression_factor", compression_factor, "compression_factor", [](AbstractControlUnit *thi, const std::string &name, const chaos::common::data::CDataWrapper &p) -> chaos::common::data::CDWUniquePtr
      {
  ...
  createProperty(
      "encode_threads", encode_thds, "encode_threads", [](AbstractControlUnit *thi, const std::string &name, const chaos::common::data::CDataWrapper &p) -> chaos::common::data::CDWUniquePtr
      {
        chaos::common::data::CDWUniquePtr ret(new chaos::common::data::CDataWrapper());
  ...
  createProperty(
      "png_strategy", png_strategy, "png_strategy", [](AbstractControlUnit *thi, const std::string &name, const chaos::common::data::CDataWrapper &p) -> chaos::common::data::CDWUniquePtr
      {
        chaos::common::data::CDWUniquePtr ret(new chaos::common::data::CDataWrapper());
...
void RTCameraBase::startGrabbing()
{
  ...
  if (stopCapture == true)
  {
    ...
    if (buffering > 0)
    {
      ...
      RTCameraBaseLDBG_ << "Waiting capture to start...";

      while (capture_exited == true)
      {
        usleep(10000);
      }
      RTCameraBaseLDBG_ << "capture started";
...
*/
  ...
  for (int cnt = 0; cnt < encode_thds; cnt++)
  {
    ...
    RTCameraBaseLDBG_ << cnt << "] Capture frames:"<<captureImg[cnt].length()<<" Encoded frames:" << encodedImg[cnt].length();
    captureImg[cnt].clear();
    encodedImg[cnt].clear();
    RTCameraBaseLDBG_ << cnt << "] After clear Capture frames:"<<captureImg[cnt].length()<<" Encoded frames:" << encodedImg[cnt].length();
...
void RTCameraBase::captureThread()
{
  ...
  RTCameraBaseLDBG_ << "Capture thread ENDED queue:" << captureImg[capture_cnt].length();
...
void RTCameraBase::encodeThread(int indx)
{
  ...
  while ((!stopCapture) && (hasStopped() == false))
  {
    ...
    if ((ret >= 0) && framebuf && framebuf->buf)
    {
      ...
      //RTCameraBaseLDBG_ << "popping image "<<isizex<<"x"<<isizey<< "("<<framebuf->offsetx<<","<<framebuf->offsety<<")"<<" encoding:"<<framebuf->frame_encoding<<" size:"<<framebuf->size<<" buf:"<<(void*)<<framebuf->buf;
      ...
      try
      {
        ...
        {
          cv::Mat matuc1 = cv::Mat(isizey, isizex, CV_8UC2, framebuf->buf);
          // cv::Mat mat16uc3_rgb(*sizey, *sizex, CV_16UC3);
          ...
          if (image.data)
          {
            ...
            else
            {
              ...
              if (res.data)
              {
                ...
                for_each(calibimages.begin(), calibimages.end(), [](const cv::Mat *ele)
                         { delete ele; });
                calibimages.clear();
