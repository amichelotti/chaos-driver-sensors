...
Il datalogging avviene mediante file EndNode5.txt
N.b.
...
#include <dht11.h>
const uint8_t dht11SensorsQty = 2; // Numero di sensori DHT11
const uint8_t dht11SensorsDIP [2] = {2,3}; // DIP cui sono connessi
dht11 dht11_1, dht11_2; // Istanzio i singoli oggetti
dht11 dht11Sensors[2] = {dht11_1, dht11_2}; // Dispongo gli oggetti in un array
...
void setup() {
  ...
  if(!SD.begin(chipSelect)) {
    Serial.println(F("*** E5 - M1")); // SD Initialization failed
    Serial.flush();
  }
//  initSession();
}
...
int CO2SensorGetPercentage(float volts, float *pcurve){
  if((volts/DcGain)>=ZeroPointVoltage)
    //return -1;
    return (-178.09*volts)+820.47;// TUNING: Era +890.47 ma da valori troppo alti Approssimo la coda dell'esponenziale negativo con un segmento con coordinate (2,754 V, 400 PPM), (5 V, o PPM)
  else
...
void storeData (String str){
  EndNode2LogFile = SD.open("EndNode5.txt", FILE_WRITE);
...
//    Serial.flush();
    ...
    Serial.println(F("*** E5 - M3"));// ERROR: Writing EndNode2.txt failed
    Serial.flush(); 
  }  
}
...
void initSession() {
  ...
  String str="*** E5 - Starting new session: ";
